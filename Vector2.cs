...
namespace LiveSplit.Kalimba {
    ...
    public struct Vector2 {
        ...
        public static Vector2 INVALID = new Vector2() { X = float.MinValue, Y = float.MinValue };
        [FieldOffset(0)]
        public float X;
        [FieldOffset(4)]
        public float Y;
        ...
        public static Vector2 operator +(int one, Vector2 two) {
            return new Vector2() { X = one + two.X, Y = one + two.Y };
        }
        public static Vector2 operator *(Vector2 one, Vector2 two) {
            return new Vector2() { X = one.X * two.X, Y = one.Y * two.Y };
        }
